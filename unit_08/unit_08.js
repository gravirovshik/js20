...
function t11() {
    ...
    let list = document.querySelectorAll('div.div-11');
    let start = 0;
    str = '';

    while (list.length > start) {
        str = str + `${list[start].innerHTML}_`;
        start++;
    }
...
function t12() {
    ...
    while (list.length > start) {
        list[start].style.background = 'orange';
        start++;
    }
}
...
function t14() {
    ...
    while (list.length > start) {
        if (list[start].checked) {
            document.querySelector('.out-14').innerHTML = list[start].value;
        }
        start++;
    }
